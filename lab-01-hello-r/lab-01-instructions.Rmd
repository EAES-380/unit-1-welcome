...
**The starter file is where you will make all your changes. You will not need to make any edits to the instructions files throughout the course.* (Unless you are fixing one of my typos ;) )
...
- **datasauRus**: containing the datasets we'll be using
- **tidyverse**: a collection of packages for doing data analysis. 
  
These packages are already **installed** for you. However, you still need to **load** them (make them active in your session). 
...
**The Green Play Button** is called the "Run Current Chunk" button. You will use it a lot. :)
...
The data we will be working with today is called `datasaurus_dozen` and it an object stored in the `datasauRus` package.  

`datasaurus_dozen` contains x and y coordinates for 13 shapes. The summary statistics for each of these shapes are the same, but the shapes look very different.   

This lab is designed to show us why data visualization is important and how summary statistics alone can be misleading.  

To find out more about `datasaurus_dozen`, navigate to your starter file, and replace the text in the `demo-code-chunk` with:  

`?datasaurus_dozen`  

**Help Files:** A question mark `?` before the name of an object will always bring up its help file.
